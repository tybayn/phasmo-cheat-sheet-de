...
function loadAllAndConnect() {
  let loadZN = new Promise((resolve, reject) => {
    if (znid && znid != "no-connection-to-server") {
      ...
      fetch(`https://zero-network.net/zn/`, {
    ...
    fetch("data/ghosts.json", { signal: AbortSignal.timeout(6000) })
      ...
      .then((data) => {
        ...
        for (var i = 0; i < data.ghosts.length; i++) {
          ...
          if (data.ghosts[i].max_speed != null) {
            bpm_speeds.add(data.ghosts[i].max_speed);
          }
          if (data.ghosts[i].alt_speed != null) {
            bpm_speeds.add(data.ghosts[i].alt_speed);
          }
  ...
  Promise.all([loadZN, loadData]).then((x) => {
    auto_link();
  });
}
