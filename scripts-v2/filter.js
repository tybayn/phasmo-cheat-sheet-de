...
const all_speed = {"Slow":"Langsam","Normal":"Normal","Fast":"Schnell"}
const all_sanity = {"Late":"Spät","Average":"Average","Early":"Früh","VeryEarly":"SehrFrüh"}
let all_evidence = []
let all_ghosts = []
let all_maps = {}
let bpm_list = []
...
var user_settings = {"num_evidences":"3","cust_num_evidences":"3","cust_hunt_length":"3","ghost_modifier":2,"volume":50,"mute_timer_toggle":0,"mute_timer_countdown":0,"timer_count_up":0,"timer_split":1,"offset":0.0,"sound_type":0,"speed_logic_type":0,"bpm":0,"domo_side":0,"priority_sort":0,"map":"6 Tanglewood Drive","theme":"Standard"}
...
function closeMenu(){
    ...
    if(mquery.matches){
        document.getElementById("menu").style.marginBottom = is_c ? "-680px" : "-625px";
...
function resetResetButton(){
    ...
    $("#reset").html(polled ? "Waiting for others..." : "Zurücksetzen")
...
function showSettings(){
    mquery = window.matchMedia("screen and (pointer: coarse) and (max-device-width: 600px)")
    if (document.getElementById("settings_box").style.left == (mquery.matches ? "calc(-60% - 40px)" : "23px")){
        ...
        document.getElementById("settings_box").style.left = (mquery.matches ? "0px" : "245px")
    }
    else {
        document.getElementById("settings_box").style.left = (mquery.matches ? "calc(-60% - 40px)" : "23px")
...
function showEvent(){
    mquery = window.matchMedia("screen and (pointer: coarse) and (max-device-width: 600px)")
    if (document.getElementById("event_box").style.left == (mquery.matches ? "calc(-60% - 40px)" : "-127px")){
        ...
        document.getElementById("event_box").style.left = (mquery.matches ? "0px" : "245px")
    }
    else {
        document.getElementById("event_box").style.left = (mquery.matches ? "calc(-60% - 40px)" : "-127px")
...
function showWiki(){
    mquery = window.matchMedia("screen and (pointer: coarse) and (max-device-width: 600px)")
    if (document.getElementById("wiki_box").style.left == (mquery.matches ? "calc(-60% - 40px)" : "-127px")){
        ...
        document.getElementById("wiki_box").style.left = (mquery.matches ? "0px" : "245px")
    }
    else {
        document.getElementById("wiki_box").style.left = (mquery.matches ? "calc(-60% - 40px)" : "-127px")
...
function showMaps(forceOpen = false, forceClose = false){
    mquery = window.matchMedia("screen and (pointer: coarse) and (max-device-width: 600px)")
    if (document.getElementById("maps_box").style.left == (mquery.matches ? "calc(-100% + 60px)" : "-333px") && !forceClose){
        ...
        document.getElementById("maps_box").style.left = (mquery.matches ? "0px" : "245px")
        document.getElementById("maps_box").style.width = (mquery.matches ? "calc(100% - 100px)" : "calc(100% - 313px)")
    }
    else if(!forceOpen) {
        document.getElementById("maps_box").style.width = (mquery.matches ? "calc(100% - 100px)" : "556px")
        document.getElementById("maps_box").style.left = (mquery.matches ? "calc(-100% + 60px)" : "-333px")
...
function flashMode(){
    ...
    var mode_text = {"-1":"Benutzerdefiniert","0":"Apokalypse III","1":"Wahnsinn","2":"Albtraum","3":"Profi","3I":"Fortgeschritten","3A":"Anfänger"}[cur_evidence]
    document.getElementById("game_mode").innerHTML = `${mode_text}<span>(${parseInt(cur_evidence)} Beweis)</span>`.replace("-1",document.getElementById("cust_num_evidence").value)
...
function loadSettings(){
    ...
    try{
        ...
        user_settings = {"num_evidences":"3","cust_num_evidences":"3","cust_hunt_length":"3","ghost_modifier":2,"volume":50,"mute_timer_toggle":0,"mute_timer_countdown":0, "timer_count_up":0,"timer_split":1,"offset":0.0,"sound_type":0,"speed_logic_type":0,"bpm_type":0,"bpm":0,"domo_side":0,"priority_sort":0,"map":"6 Tanglewood Drive","theme":"Standard"}
    }
...
function showCustom(){
    ...
    if(document.getElementById("num_evidence").value == "-1"){
        if(mquery.matches){
            document.getElementById("menu").style.height="715px";
            if(is_h){
                document.getElementById("menu").style.marginBottom = "-680px";
                $("#domovoi").removeClass("domovoi-custom")
                $("#domovoi").addClass("domovoi-menu-hidden")
            }
            else{
        ...
        if(mquery.matches){
            if(is_h){
                document.getElementById("menu").style.marginBottom = "-625px";
                $("#domovoi").removeClass("domovoi-custom")
                $("#domovoi").addClass("domovoi-menu-hidden")
            }
            else{
            ...
            document.getElementById("menu").style.height="660px";
        }
    }
}
